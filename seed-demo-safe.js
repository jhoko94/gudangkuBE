// Script seed yang aman untuk production
// Hanya seed jika data belum ada (untuk start command)
...
async function main() {
    ...
    if (existingPemasok > 0 || existingTujuan > 0 || existingBarang > 0) {
        ...
        console.log(`   - Pemasok: ${existingPemasok}`);
        console.log(`   - Tujuan: ${existingTujuan}`);
        console.log(`   - Barang: ${existingBarang}`);
        return;
    }

    console.log('🌱 Database kosong, mulai seeding master data demo...\n');
...
main()
    .catch((e) => {
        ...
        // Jangan exit dengan error, biarkan server tetap start
        process.exit(0);
    })
    .finally(async () => {
